...
async def cmd_help(message: Message):
    text = (
        "📌 Bot buyruqlari (FAQAT ADMIN):\n"
        "/mymembers — Siz qo‘shgan odamlar soni\n"
        "/yourmembers — Reply qilingan odam qo‘shganlar soni\n"
        "/top — Eng ko‘p odam qo‘shgan TOP 10\n"
        "/delson — Guruh statistikani tozalash\n"
        "/clean — Reply qilingan odamni 0 qilish\n"
        "/deforce — Majburiy sozlamalarni default\n"
        "/priv — Reply qilib imtiyoz berish\n"
        "/privs — Imtiyoz ro‘yxati\n"
        "/setchannel — Kanal boshqaruvi\n"
    )
...
# ---------- PRIV GIVE (reply) ----------
@group_router.message(AdminOnly(), Command("priv"))
async def cmd_priv(message: Message, db: DB):
    if not message.reply_to_message or not message.reply_to_message.from_user:
        return await message.reply("❗ Odamga reply qilib /priv yozing.")
    uid = message.reply_to_message.from_user.id
    await db.add_priv(message.chat.id, uid)
    await message.reply("✅ Imtiyoz berildi. (U odam majburiy qo‘shishsiz yozadi)")

# ---------- PRIV LIST ----------
@group_router.message(AdminOnly(), Command("privs"))
async def cmd_privs(message: Message, db: DB):
    ids = await db.list_priv(message.chat.id)
    if not ids:
        return await message.reply("📃 Imtiyoz ro‘yxati bo‘sh.")
    lines = ["📃 Imtiyoz berilganlar:"]
    for uid in ids[:50]:
        lines.append(f"• <a href='tg://user?id={uid}'>User</a>")
    await message.reply("\n".join(lines))


print("✅ handlers_group.py LOADED")
...
async def is_subscribed_all(bot, user_id: int, channels: list[str]) -> bool:
    for ch in channels:
        ...
        try:
            ...
            # kanal topilmadi / bot admin emas / private / huquq yetarli emas
            return False
    return True
...
async def sc_save_channel(message: Message, db: DB):
    # oddiy user @... yozib yuborsa ham qo‘shilib ketmasin
    if not await AdminOnly()(message):
        return
...
# =========================
#   STATISTIKA: yangi odam qo‘shilganda
# =========================
...
async def on_new_members(message: Message, db: DB):
    ...
    if not inviter:
        return
    await db.inc_added(message.chat.id, inviter.id, len(message.new_chat_members))
...
async def text_time_menu(message: Message, db: DB):
    s = await db.get_settings(message.chat.id)
    await message.reply("Matn avtomatik o‘chish vaqti:", reply_markup=kb_text_delete_time(selected_seconds=s["force_text_delete_after"]))
